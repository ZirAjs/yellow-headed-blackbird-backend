...
class NatureEventsView(APIView):
    @swagger_auto_schema(
        manual_parameters=[
            openapi.Parameter(
                "start_date_time",
                openapi.IN_QUERY,
                description="시작 날짜시간 (예: 2025-05-10T22:00:00)",
                type=openapi.TYPE_STRING,
            ),
            openapi.Parameter(
                "end_date_time",
                openapi.IN_QUERY,
                description="끝 날짜시간 (예: 2025-05-11T04:00:00)",
                type=openapi.TYPE_STRING,
            ),
        ],
    ...
    def get(self, request):
        ...
        start_dt = start_dt.replace(tzinfo=None) if start_dt else None
        end_dt = end_dt.replace(tzinfo=None) if end_dt else None
        ...
        try:
            ...
            return Response(
                {"error": "Invalid base_day format. Use YYYY-MM-DD."}, status=400
            )

        lat = float(request.GET.get("lat", 37.5665))
        lng = float(request.GET.get("lng", 126.9780))

        sun_data = fetch_sun_info(
            lat=lat, lng=lng, base_date=str(base_date), next_date=str(next_date)
        )
